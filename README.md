...
| **Parakeet** | NVIDIA's accurate STT with auto language detection | EN (v2), 25 EU languages (v3) | [mlx-community/parakeet-tdt-0.6b-v3](https://huggingface.co/mlx-community/parakeet-tdt-0.6b-v3) |
...
NVIDIA's high-accuracy speech-to-text model. Parakeet v3 supports 25 European languages with automatic language detection.
...
# Transcribe audio - language is automatically detected
result = model.generate("audio.wav")
print(f"Text: {result.text}")
print(f"Detected language: {result.language}")  # e.g., "en", "de", "fr"
...
for chunk in model.generate("long_audio.wav", stream=True):
    print(f"[{chunk.language}] {chunk.text}", end="", flush=True)
```
