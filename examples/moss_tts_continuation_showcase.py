...
- final `assistant` message carries prefix audio (`<|audio|>`) plus continuation text
...
def main() -> None:
    ...
    assistant_content = "<|audio|>"
    if args.user_text and args.user_text.strip():
        assistant_content = f"{assistant_content} {args.user_text.strip()}"
