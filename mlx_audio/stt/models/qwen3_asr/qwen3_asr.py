...
class AudioEncoder(nn.Module):
    ...
    def __call__(
        ...
        padded_mask_after_cnn = np.zeros((len(chunk_lengths), max_len_after_cnn), dtype=bool)
...
class Model(nn.Module):
        ...
        if input_features is not None and (cache is None or cache[0] is None or cache[0].offset == 0):
            ...
            if audio_token_mask.any():
                ...
                if len(audio_indices) > 0 and audio_features.shape[0] > 0:
                    ...
                    inputs_embeds = mx.stack(result_list, axis=0).reshape(batch_size, seq_len, hidden_dim)
    ...
    def _build_prompt(self, num_audio_tokens: int, language: str = "English") -> mx.array:
    ...
    def stream_generate(
        ...
        *,
        ...
        pbar = tqdm(total=max_tokens, desc="Generating", unit="tok", disable=not verbose, leave=False)
    ...
    def generate(
        ...
        **kwargs,
        ...
        return STTOutput(
            ...
            prompt_tps=prompt_tokens / (end_time - start_time) if end_time > start_time else 0,
            generation_tps=len(generated_tokens) / (end_time - start_time) if end_time > start_time else 0,
        )
