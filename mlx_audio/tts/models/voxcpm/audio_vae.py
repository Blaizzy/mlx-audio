...
class CausalConv1d(nn.Conv1d):
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int, stride: int = 1, dilation: int = 1, padding: int = 0, groups: int = 1, bias: bool = True):
        super().__init__(
...
class CausalResidualUnit(nn.Module):
    def __init__(self, dim: int = 16, dilation: int = 1, kernel: int = 7, groups: int = 1):
        ...
        self.conv1 = CausalConv1d(dim, dim, kernel_size=kernel, dilation=dilation, padding=pad, groups=groups)
...
class CausalEncoder(nn.Module):
    def __init__(self, d_model: int = 64, latent_dim: int = 32, strides: List[int] = [2, 4, 8, 8], depthwise: bool = False):
        super().__init__()
        ...
        for stride in strides:
            ...
            self.blocks.append(CausalEncoderBlock(output_dim=next_dim, input_dim=curr_dim, stride=stride, groups=groups))
            curr_dim = next_dim
...
class CausalDecoderBlock(nn.Module):
    def __init__(self, input_dim: int = 16, output_dim: int = 8, stride: int = 1, groups: int = 1, use_noise_block: bool = False):
        super().__init__()
...
class CausalDecoder(nn.Module):
    def __init__(self, input_channel: int, channels: int, rates: List[int], depthwise: bool = False, d_out: int = 1, use_noise_block: bool = False):
        super().__init__()
        
        if depthwise:
            self.conv_in = nn.Sequential(
                CausalConv1d(input_channel, input_channel, kernel_size=7, padding=3, groups=input_channel),
                CausalConv1d(input_channel, channels, kernel_size=1)
            )
        else:
            self.conv_in = CausalConv1d(input_channel, channels, kernel_size=7, padding=3)
        ...
        for i, stride in enumerate(rates):
            ...
            self.blocks.append(CausalDecoderBlock(input_dim, output_dim, stride, groups, use_noise_block))
            
        self.blocks = nn.Sequential(*self.blocks)
        final_dim = channels // (2**len(rates)) # This matches the last output_dim in loop
        ...
        for k, v in fused_weights.items():
            ...
            if parts[0] == "encoder":
                if parts[1] == "block":
                    ...
                    if idx == 0:
                        ...
                        new_parts = ["encoder", "blocks", "layers", str(idx-1)] + parts[3:]
                else:
            ...
            # Decoder
                ...
                if parts[1] == "model":
                    ...
                    if idx == 0:
                        ...
                        new_parts = ["decoder", "blocks", "layers", str(idx-2)] + parts[3:]
            ...
            while i < len(new_parts):
                ...
                if p == "block" and i+1 < len(new_parts) and new_parts[i+1].isdigit():
                    ...
                    is_encoder_block = "encoder" in new_parts[:i] and "blocks" in new_parts[:i]
                    is_decoder_block = "decoder" in new_parts[:i] and "blocks" in new_parts[:i]
                    ...
                    if is_encoder_block and len(final_parts) == 4: 
                        ...
                        mapping = {0: "res1", 1: "res2", 2: "res3", 3: "snake", 4: "conv"}
                    ...
                    if is_decoder_block and len(final_parts) == 4:
                        ...
                        mapping = {0: "snake", 1: "conv_t", 2: "res1", 3: "res2", 4: "res3"}
                        final_parts.append(mapping.get(idx, f"unknown_{idx}"))
                        i += 2
                        continue
